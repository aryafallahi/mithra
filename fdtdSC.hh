/********************************************************************************************************
 *  fdtdSC.hh : Implementation of the real fdtd time marching solution class for the darius code
 ********************************************************************************************************/
...
namespace Darius
{
  ...
  public:
    ...
    FdTdSC( Mesh& mesh, Bunch& bunch, Seed& seed, Undulator& undulator, std::vector<ExtField>& extField, std::vector<FreeElectronLaser>& FEL )
      ...
      while (time_ < mesh_.totalTime_)
	{
	  ...
	  if (seed_.profile_)
		...
		if ( time_ - seed_.profileTime_[i] < mesh_.timeStep_ && time_ > seed_.profileTime_[i] )
	      ...
	      if ( ( fmod(time_,seed_.profileRhythm_) < mesh_.timeStep_ ) && ( time_ > 0.0 ) )
		fieldProfile();
	    }
	...
	for ( pf_.j = 0; pf_.j < N1_; pf_.j++ )
	  for ( pf_.k = ( (rank_ == 0) ? 0 : 1 ); pf_.k < np_ - ( ( rank_ == size_  - 1 ) ? 0 : 1 ); pf_.k++ )
	      ...
	      for (pf_.l = 0; pf_.l < seed_.profileField_.size(); pf_.l++)
		{
		  ...
		  else if 	( seed_.profileField_[pf_.l] == F  )
		    *pf_.file << (*fn_)[pf_.m] << "\t";
		  else if 	( seed_.profileField_[pf_.l] == Q  )
		    *pf_.file << rn_[pf_.m] << "\t";
		}
