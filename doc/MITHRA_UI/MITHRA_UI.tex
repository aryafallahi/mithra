...
\begin{itemize}
	...
	%
	%
	\item {\tt \em \small lorentz-factor} is a the Lorentz factor of the moving mesh. If this value is not given the solver automatically computes an optimal value. It is generally recommended to leave this option so that the solver does the automatic computation. However, in some rare cases setting manually the Lorentz factor of the mesh is needed.
\end{itemize}
...
%
\item {\tt \small \em file-name} returns the name of the file where the bunch data is written for a bunch of type {\tt \small \em file}. If the file does not reside in the current directory file-name should contain the file address.
%
...
%
\begin{itemize}
	\item {\tt \small \em type} is the type of the excitation or the seed field. The accepted excitation types in MITHRA include plane wave, truncated plane-wave, Gaussian beam, and super-Gaussian beam. A truncated plane-wave is a plane-wave that introduces fields to the particle only over an ellipse determined by beam radii.
	%
	...
	%
	\item {\tt \small \em radius-parallel} is the Rayleigh radius (beam waist) of the Gaussian beam in the direction parallel to the polarization. For the truncated plane-wave it is the radius of the ellipse along the polarization direction confining the plane wave.
	%
	\item {\tt \small \em radius-perpendicular} is the Rayleigh radius (beam waist) of the Gaussian beam in the direction perpendicular to the polarization. For the truncated plane-wave it is the radius of the ellipse perpendicular to the polarization direction confining the plane-wave.
	%
	\item {\tt \small \em order-parallel} is the order of the super Gaussian beam along the field polarization.
	%
	\item {\tt \small \em order-perpendicular} is the order of the super Gaussian beam perpendicular to the field polarization.
	%
	...
	%
	\begin{enumerate}
		\item {\tt \small \em neumann:}
		%
		\begin{equation}
			f(t) = \displaystyle - A_0 4 \ln 2 \: \cos( 2 \pi f (t - t_0) + \phi_\mathrm{CEP} ) \frac{t - t_0}{\tau^2} e^{-2 \ln 2 \: (t - t_0)^2/\tau^2 }
			\label{neumann}
		\end{equation}
		%
		\item {\tt \small \em gaussian:}
		%
		\begin{equation}
			f(t) = \displaystyle A_0 \cos( 2 \pi f (t - t_0) + \phi_\mathrm{CEP} ) e^{-2 \ln 2 \: (t - t_0)^2/\tau^2 }
			\label{gaussian}
		\end{equation}
	    %
		\item {\tt \small \em secant-hyperbolic:}
		%
		\begin{equation}
			f(t) = \displaystyle A_0 \cos( 2 \pi f (t - t_0) + \phi_\mathrm{CEP} ) \frac{1}{\cosh ( (t - t_0)/\tau ) }
			\label{secantHyperbolic}
		\end{equation}
		%
		\item {\tt \small \em flat-top:}
		%
		\begin{equation}
			f(t) = \displaystyle A_0 \cos( 2 \pi f (t - t_0) + \phi_\mathrm{CEP} ) \left\{ \begin{array}{ll} e^{- (t - t_0-\tau/2)^2/(N/f)^2} & \|t-t_0\| \ \geq \tau/2 \\ 1.0 & \|t-t_0\| \ \leq \tau/2 \end{array} \right.
			\label{flattop}
		\end{equation}
	    %
	    \item {\tt \small \em inverse-gaussian:}
	    %
	    \begin{equation}
	    	f(t) = \displaystyle A_0 \cos( 2 \pi f (t - t_0) + \phi_\mathrm{CEP} ) \sqrt[4]{ \left(1+\frac{t^2}{\sigma_1^2}\right) \left(1+\frac{t^2}{\sigma_2^2}\right)}  \left\{ \begin{array}{ll} e^{- (t - t_0-\tau/2)^2/(N/f)^2} & \|t-t_0\| \ \geq \tau/2 \\ 1.0 & \|t-t_0\| \ \leq \tau/2 \end{array} \right.
	...
	%
	\item {\tt \small \em rising-cycles} is the number of cycles that it takes for the flat-top and inverse-Gaussian pulses to reach to the signal value in the corresponding time interval, i.e. $N$ in (\ref{flattop}) and (\ref{inverseGaussian}). Default value is two cycles.
	%
	\item {\tt \small \em CEP} is the carrier envelope phase $\phi_{\mathrm{CEP}}$ of the modulated signal in degrees.
	%
	\item {\tt \small \em sigma-inverse-gaussian} is a vector of two real numbers containing the values of parameters $\sigma_1$ and $\sigma_2$ for the inverse-Gaussian pulse in (\ref{inverseGaussian}).
\end{itemize}
%
...
\{
	...
	\{
		...
		\textcolor{red}{field} 					\textcolor{red}{ = < Ex | Ey | Ez | Bx | By | Bz | Ax | Ay | Az | F >}
		directory  				 = < /path/to/location >
		base-name  				 = < string >
		rhythm  				 = < real >
		\textcolor{red}{position}  				 \textcolor{red}{= < ( real, real, real ) >}
...
\begin{itemize}
	...
	\item {\tt \small \em pulse-length} is the pulse duration of the signal in length units $c\tau$. The pulse duration is defined as the interval in which the pulse intensity is larger than half the maximum intensity of the pulse, i.e. FWHM of the intensity.
	%
	\item {\tt \small \em wavelength} is the modulation wavelength $\lambda_0$ of the modulated signal. Default value is two cycles.
	%
	\item {\tt \small \em rising-cycles} is the number of cycles that it takes for the flat-top and inverse-Gaussian pulses to reach to the signal value in the corresponding time interval, i.e. $N$ in (\ref{flattop}) and (\ref{inverseGaussian}). Default value is two cycles.
	%
	...
	\item {\tt \small \em sigma-inverse-gaussian} is a vector of two real numbers containing the values of parameters $\sigma_1$ and $\sigma_2$ for the inverse-Gaussian pulse in (\ref{inverseGaussian}).
	%
	\item {\tt \small \em distance-to-bunch-head} determines the distance between the reference position of the optical undulator and the bunch head at the initialization time. This distance is needed to avoid particles experiencing a sudden change in the undulator field. The value of this parameter is by default five time the undulator periods for optical undulators. In MITHRA, the radiation of the particles are calculated after they pass through this point. Therefore, this value has a different effect compared to the {\tt \small \em initial-time-back-shift} parameter that only shifts the particle in time.
\end{itemize}
%
\end{enumerate}
...
\begin{itemize}
	%
    \item {\tt \small \em beam-type} is the type of the beam. The accepted excitation types in MITHRA include plane wave, truncated plane-wave, Gaussian beam, and super-Gaussian beam. A truncated plane-wave is a plane-wave that introduces fields to the particle only over an ellipse determined by beam radii. In addition, for each beam type an equivalent standing wave type can be defined, which represents superposition of two beams with same properties propagating counter to each other.
    %
	...
	\item {\tt \small \em wavelength} is the modulation wavelength $\lambda_0$ of the modulated signal.
	%
	\item {\tt \small \em rising-cycles} is the number of cycles that it takes for the flat-top and inverse-Gaussian pulses to reach to the signal value in the corresponding time interval, i.e. $N$ in (\ref{flattopE}) and (\ref{inverseGaussianE}). Default value is two cycles.
	%
	\item {\tt \small \em CEP} is the carrier envelope phase $\phi_{\mathrm{CEP}}$ of the modulated signal in degrees.
	%
	\item {\tt \small \em sigma-inverse-gaussian} is a vector of two real numbers containing the values of parameters $\sigma_1$ and $\sigma_2$ for the inverse-Gaussian pulse in (\ref{inverseGaussianE}).
	%}
\end{itemize}
\end{enumerate}
